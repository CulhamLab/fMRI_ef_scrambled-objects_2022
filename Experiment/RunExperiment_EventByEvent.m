...
p.TIMING.TIME_BEFORE_END_OF_EVENT_TO_STOP_UPDATING_DISPLAY = 0.005;
...
if abs(d.total_vol - round(d.total_vol)) < 1e-10
    d.total_vol = round(d.total_vol);
end

d.total_events = height(d.order);
...
d.events = table('Size',[d.total_events size(name_types,1)],'VariableNames',name_types(:,1),'VariableTypes',name_types(:,2));
% d.events.Volume = (1:d.total_vol)';
d.events.Trial(:) = nan;
d.events.DisplayEvent(:) = nan;
d.events.TimeStartEffective(:) = nan;
d.events.TimeStartActual(:) = nan;
d.events.TimeEnd(:) = nan;
d.events.DurationActual(:) = nan;
d.events.DurationEffective(:) = nan;
d.events.ReceivedTrigger(:) = false;
d.events.ButtonPress(:) = false;
d.events.ButtonPressTime(:) = nan;
...
d.display_events = table('Size',[d.total_events size(name_types,1)],'VariableNames',name_types(:,1),'VariableTypes',name_types(:,2));
d.display_events.EventID = (1:d.total_events)';

%add events from order
vol = 0;
for e = 1:d.total_events
...
%         vol = vol+1;
        ...
        %duration
        d.events.Duration(e) = d.order.Duration_Seconds(e);
        
        %start in run
        d.events.TimeStartEffective(e) = sum(d.order.Duration_Seconds(1:(e-1)));

        %trial
        d.events.Trial(e) = d.order.Trial(e);
        
        %condition
        d.events.Condition{e} = d.order.Condition{e};
        
        %display event
        d.events.DisplayEvent(e) = e;
%     end
...
%% 3D Test Image
msg = sprintf('Waiting for trigger (%d events, %g sec)', d.total_events, d.total_sec);
...
evt = nan;
event_start = 1;
time_last_trigger = t0;
for e = 1:d.total_events%v = vol_start:d.total_vol
    %volume start time actual
    if e==event_start
        d.events.TimeStartActual(e) = 0;
    else
        d.events.TimeStartActual(e) = GetSecs - t0;
    end
    
%     %volume start time
%     if e==event_start || d.events.ReceivedTrigger(e-1) %is first vol OR prior vol recieved trigger
%         d.events.TimeStartEffective(e) = d.events.TimeStartActual(e); %use actual time
%     else %missed a trigger
%         d.events.TimeStartEffective(e) = d.events.TimeStartEffective(e-1) + p.TR; %use expected trigger time
%     end
    
    %event?
    if evt ~= d.events.DisplayEvent(e)
        evt = d.events.DisplayEvent(e);
        time_event_start = d.events.TimeStartEffective(e);
    end
    
    %start message
    fprintf('\nStarting event %d/%d at %fsec (actual %fsec):\n',e,d.total_events,d.events.TimeStartEffective(e),d.events.TimeStartActual(e));
    fprintf('\tCondition: %s\n', d.events.Condition{e});
    fprintf('\tDuration: %g\n', d.events.Duration(e));
    
    %volume events
    saved = false;
    while 1
        time_in_vol = (GetSecs-t0) - d.events.TimeStartEffective(e);
...
%         end
        ...
        %time up?
        if time_in_vol >= d.events.Duration(e)
            break;
        end
        ...
        [keyIsDown, ~, keyCode] = KbCheck(-1); %get key(s)
        if keyIsDown
            if any(keyCode(p.KEY.TRIGGER)) && ((GetSecs - time_last_trigger) > (p.TR/2))
                d.events.ReceivedTrigger(e) = true;
                d.events.TriggerTime{e}(end+1) = (GetSecs-t0);
                time_last_trigger = GetSecs;
                fprintf('~~~~~~~~~~~~~~~~~TRIGGER RECIEVED~~~~~~~~~~~~~~~~~\n')
            elseif any(keyCode(p.KEY.STOP))
                error('Stop key was pressed.') 
            elseif ~d.events.ButtonPress(e) && any(keyCode(p.KEY.BUTTON_BOX))
                d.events.ButtonPress(e) = true;
                d.events.ButtonPressTime(e) = time_in_vol;
                fprintf('-Button Box\n');
            end
        end
        
        %can do display?
        if time_in_vol < (d.events.Duration(e) - p.TIMING.TIME_BEFORE_END_OF_EVENT_TO_STOP_UPDATING_DISPLAY)
            ...
            if isnan(d.display_events.Events{evt}.TimeInEventDisplayed(ind))
                ...
                d.display_events.Events{evt}.VolDisplayed(ind) = e;
                d.display_events.Events{evt}.TimeInVolDisplayed(ind) = time_in_vol;
            elseif ~saved && ((d.events.Duration(e) - time_in_vol) > 0.5)
                %save
                saved = true;
    ...
    d.events.TimeEnd(e) = GetSecs-t0;
    d.events.DurationEffective(e) = d.events.TimeEnd(e) - d.events.TimeStartEffective(e);
    d.events.DurationActual(e) = d.events.TimeEnd(e) - d.events.TimeStartActual(e);
    fprintf('-duration: %f seconds\n',d.events.DurationEffective(e))
